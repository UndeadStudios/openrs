...
public class ItemDefDumperExtPatrickNew {
	
	public static final String EXPORT = "E:/dump/export/ItemDefsPatrick.json";
	public static final String EXPORT2 = "E:/dump/export/EquipmentDefsPatrick.json";
	
	public static final String LISTFILE = "./repository/types/items.txt";
	
	public static final String[] TYPES = new String[] { "HAT", "CAPE", "AMULET", "WEAPON", "BODY", "SHIELD", "", "LEGS", "", "GLOVES", "BOOTS", "", "RING", "ARROWS" };
	
	public static final String[] SKILLS = new String[] {"ATTACK", "DEFENCE", "STRENGTH", "HITPOINTS", "RANGED", "PRAYER", "MAGIC", 
			  "COOKING", "WOODCUTTING", "FLETCHING", "FISHING", "FIREMAKING", "CRAFTING", "SMITHING", "MINING", "HERBLORE", "AGILITY", "THIEVING",
	...
	public static void main(String[] args) throws Exception {
		...
		JsonWriter json2 = new JsonWriter(new FileWriter(EXPORT2));
		json2.setIndent("  ");
		json2.beginArray();
			    	...
			    	if(document != null) {
			            BufferedWriter htmlWriter = 
			            	     new BufferedWriter(new OutputStreamWriter(
			            	    		 new FileOutputStream(".-master/RS/wiki/" +
...
//				        					System.out.println("----");
			    	...
			    	if(item.id != -1) {
			    		...
			    		json.name("parentId");
			    		json.value(item.note ? item.unnotedId : -1);
			    		json.name("notedId");
			    		json.value(item.note ? -1 : item.notedId);
			    		json.name("stackable");
			    		json.value(item.stackable);
			    		json.name("destroyable");
			    		json.value(false);
			    		json.name("tradeable");
			    		...
			    		json.name("equipable");
			    		json.value(item.slot != -1);
			    		json.name("weapon");
			    		json.value(item.slot == 3);
			    		json.name("weight");
			    		json.value(item.weight);
			    		json.name("special_value");
			    		json.value(0);
			    		json.name("fullHelm");
			    		...
			    		if (item.equipable && item.slot != -1) {
				    		...
				    		json2.value(item.id);
				    		json2.name("type");
				    		json2.value(TYPES[item.slot]);
				    		json2.name("name");
				    		json2.value(item.name);
				    		json2.name("requirements");
				    		json2.beginArray();
				    		for (int i = 0; i < item.requirements.length; i++) {
				    			if (item.requirements[i] != 0) {
				    				json2.beginObject();
				    				json2.name("level");
						    		json2.value(item.requirements[i]);
						    		json2.name("skill");
						    		json2.value(SKILLS[i]);
				    				json2.endObject();
				    			}
				    		}
				    		json2.endArray();
