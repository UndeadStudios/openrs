...
public class ItemDefDumperFred {
	
	public static final String EXPORT = "E:/dump/export/ItemDefinitionsFred.json";
...
//				        					System.out.println("----");
			    	...
			    	if(item.id != -1) {
			    		...
			    		json.name("store-price");
			    		json.value(0);
			    		json.name("osrs-price");
			    		json.value(item.geprice);
			    		json.name("drop-value");
			    		...
			    		json.beginArray();
			    		for(int i = 0; i < 7; i++) {
			    			json.value(item.requirements[i]);
			    		}
			    		json.endArray();
