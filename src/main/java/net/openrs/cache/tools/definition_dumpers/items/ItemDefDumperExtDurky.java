...
public class ItemDefDumperExtDurky {
	
	public static final String EXPORT = "E:/dump/export/ItemDefinitionsDurky.json";
				        	...
				        	if(examineNext) {
				        		...
				        		if(text.contains("Members only")) {
				        			...
				        			if(gemain.text().contains("dose")) {
				        				...
				        				item.geprice = Integer.parseInt(ge.first().text().replace(",", ""));
				        			}
				        			...
				        			if(slotElement.last() != null) {
			        					switch(slotElement.last().attr("title").replaceAll(" slot table", "")) {
			        						...
			        						case "Body":
			        							item.slot = "CHEST";
			        							if(item.name.toLowerCase().contains("body") || item.name.toLowerCase().contains("robe")  || item.name.toLowerCase().contains("robetop") || item.name.toLowerCase().contains("chestplate")) {
			        								item.body = true;
			        							}
			        							break;
			        						case "Shield":
...
//			    		json.value(Math.round(item.highAlch * 1.6));
			    		json.name("shop-value");
			    		json.value((int) ((double)item.highAlch / 6 * 10));
			    		json.name("low-alch-value");
			    		json.value(item.lowAlch);
			    		json.name("high-alch-value");
