package net.openrs.cache.tools.definition_dumpers;
...
public class RangedBonusDumper {
	
	public static final String EXPORT = "E:/dump/export/RangedBonusses.json";
...
//			    		}
				        ...
				        for (org.jsoup.nodes.Element infoBox : list) {
				        	...
				        	if(examineNext) {
				        	...
				        	} else if(text.contains("Buy limit") && !text.contains("Varies") && !text.contains("unknown") && !text.contains("Unknown") && !text.contains("Various") && !text.contains("N/A") && !text.contains("herbs")) {
				        		text = infoBox.children().last().text();
				        		if(text.contains("doses")) {
				        			text = text.split("doses: ")[1];
				        			if(text.split(",").length > 1) {
				        		...
				        		item.buyLimit = Integer.parseInt(text.split("coins")[0].split("coin")[0].split("\\(")[0].replaceAll("Clean:", "").replaceAll("coins", "").replaceAll("coin", "").trim().replace(",", "").replaceAll("\u00A0", ""));				        		
...
//				    		json.endObject();
	...
	//			    		json.name("shopValue");
	//			    		json.value(Math.round(item.highAlch * 1.5));
