...
public class ItemDefDumperExtSalad {
	
	public static final String EXPORT = "E:/dump/export/ItemDefinitionsSalad.json";
...
//				        					System.out.println("----");
			    	...
			    	if(item.id != -1) {
			    		...
			    		json.name("coin_value");
			    		json.value(Math.round(((double) item.highAlch / 6) * 10));
			    		json.name("item_wear_slot");
			    		json.value(item.slot);
			    		
			    		json.name("item_variables");
			    		json.beginObject();
			    		json.name("is_wearable");
			    		json.value(item.equipable);
			    		json.name("is_stackable");
			    		json.value(item.stackable);
			    		json.name("is_noteable");
			    		json.value(item.noteable);
			    		json.name("is_tradeable");
			    		json.value(item.tradable);
			    		json.name("is_rare");
			    		json.value(false);
			    		json.name("is_noted_version");
			    		json.value(item.note);
			    		json.name("is_2_handed");
			    		json.value(item.twoHanded);
			    		json.name("is_full_hat");
			    		json.value(item.helm);
			    		json.name("is_full_mask");
			    		json.value(item.mask);
			    		json.name("is_full_body");
			    		json.value(item.body);
			    		json.endObject();
			    		
			    		json.name("item_bonuses");
			    		json.beginObject();
			    		json.name("attack_stab");
			    		json.value(item.bonusses[0]);
			    		json.name("attack_slash");
			    		json.value(item.bonusses[1]);
			    		json.name("attack_crush");
			    		json.value(item.bonusses[2]);
			    		json.name("attack_magic");
			    		json.value(item.bonusses[3]);
			    		json.name("attack_range");
			    		json.value(item.bonusses[4]);
			    		json.name("defence_stab");
			    		json.value(item.bonusses[5]);
			    		json.name("defence_slash");
			    		json.value(item.bonusses[6]);
			    		json.name("defence_crush");
			    		json.value(item.bonusses[7]);
			    		json.name("defence_magic");
			    		json.value(item.bonusses[8]);
			    		json.name("defence_range");
			    		json.value(item.bonusses[9]);
			    		json.name("other_strength");
			    		json.value(item.bonusses[10]);
			    		json.name("other_prayer");
			    		json.value(item.bonusses[11]);
			    		json.name("ranged_strength");
			    		json.value(item.bonusses[12]);
			    		json.name("magic_strength");
			    		json.value(item.bonusses[13]);
			    		json.endObject();
			    		
			    		json.name("level_requirements");
			    		json.beginObject();
			    		json.name("attack_level");
			    		json.value(item.requirements[0]);
			    		json.name("defence_level");
			    		json.value(item.requirements[1]);
			    		json.name("strength_level");
			    		json.value(item.requirements[2]);
			    		json.name("hitpoints_level");
			    		json.value(item.requirements[3]);
			    		json.name("range_level");
			    		json.value(item.requirements[4]);
			    		json.name("prayer_level");
			    		json.value(item.requirements[5]);
			    		json.name("magic_level");
			    		json.value(item.requirements[6]);
			    		json.endObject();
