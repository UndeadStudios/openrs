			    	...
			    	if(document != null) {
			            BufferedWriter htmlWriter = 
			            	     new BufferedWriter(new OutputStreamWriter(
			            	    		 new FileOutputStream("C:/Users/sgsrocks/Desktop/openrs/repository/wiki/" +
...
//				        					System.out.println("----");
			    	...
			    	if(item.id != -1) {
			    		...
			    		json.name("general-price");
			    		json.value(Math.round((item.highAlch / 6) * 10));
			    		json.name("special-price");
			    		json.value(item.geprice);
			    		json.name("noteable");
			    		json.value(!item.note && item.notedId > 0);
			    		json.name("noted");
	...
	private static org.jsoup.nodes.Document openDocument(String url) throws IOException {
		File input = new File("C:/Users/sgsrocks/Desktop/openrs/repository/wiki/" +
