...
public class ItemDefDumperExtEmulous {
	
	public static final String EXPORT = "E:/dump/export/ItemDefinitionsEmulous.json";
	...
	public static void main(String[] args) throws Exception {
		...
		List<String> confcodes = new ArrayList<>();
		int nullcode = 0;
...
//				        					System.out.println("----");
			    	...
			    	item.maleModel = true;
			    } else if(line.contains("type.interfaceOptions")) {
			    	line = line.replace("type.interfaceOptions = new String[] {", "");
			    	line = line.replace("};", "").replace("\"", "");
			    	item.options = line.split(",");			    	
			    	...
			    	if(item.id != -1) {
			    		...
			    		json.value(item.id);
			    		json.name("conf");
			    		if (item.name.toLowerCase().equals("null")) {
			    			json.value(item.name.replace(" ", "_").replace("'", "").toLowerCase() + (nullcode == 0 ? "" : "_" + (nullcode)));
			    			nullcode++;
			    		} else {
			    			String conf = item.name.replace(" ", "_").replace("'", "").toLowerCase() + (item.note ? "_note" : "");
				    		int confAmount = 0;
				    		boolean finish = false;
				    		while (!finish) {
				    			if (!confcodes.contains(conf + (confAmount == 0 ? "" : "_" + (confAmount))))
				    				finish = true;
				    			else
				    				confAmount++;
				    		}
				    		json.value(conf + (confAmount == 0 ? "" : "_" + (confAmount)));
				    		confcodes.add(conf + (confAmount == 0 ? "" : "_" + (confAmount)));
			    		}
			    		json.name("name");
			    		json.value(item.name);
			    		json.name("desc");
			    		json.value(item.examine.replace("�", "").replace("�", "..."));
			    		if (item.cost != 0 && !item.note) {
				    		json.name("cost");
				    		json.value(item.cost);
			    		}
			    		if (item.options != null) {
			    			for (int i = 0; i < item.options.length; i++) {
					    		if (!item.options[i].contains("null")) {
					    			json.name("iop" + (i + 1));
					    			json.value(item.options[i].trim());
					    		}
					    	}
			    		}
			    		if (item.slot != -1) {
			    			json.name("equip_slot");

							String slotName = "";
							
							switch (item.slot) {
							    case 0:
							    	if (item.name.toLowerCase().contains("med")) {
							    		slotName = "medhelm";
									} else if (item.helm) {
							    		slotName = "fullhelm";
							    	} else if (item.mask) {
			    		...
			    		String[] BONUS_CONFIG_FIELD_NAMES = {
			    				// Accuracy bonuses
			    				...
			    				// Other bonuses
			    				"bonus_strength", "", "", "bonus_prayer"
			    			};
			    		
			    		for (int i = 0; i < item.bonusses.length; i++) {
			    			if (i != 11 && i != 12) {
				    			if (item.bonusses[i] != 0) {
				    				json.name(BONUS_CONFIG_FIELD_NAMES[i]);
						    		json.value(item.bonusses[i]);
				    			}
			    			}
			    		}
