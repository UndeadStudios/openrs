...
public class ItemDefDumperExtVELDAHAR {
	
	public static final String EXPORT = "E:/dump/export/ItemDefinitionsVELDAHAR.json";
		...
		try (BufferedReader br = new BufferedReader(new FileReader(LISTFILE))) {
		    ...
		    while (!finished) {
			    ...
			    if(line.startsWith("case ")) {
			    	...
			    	String url = "http://oldschoolrunescape.fandom.com/wiki/" + item.name.replaceAll(" ", "_");
			    	...
			    	try {
			        	...
			        	if(item.name.contains("Burnt ")) {
			        		...
			        		url = "http://oldschoolrunescape.fandom.com/wiki/" + item.name.replaceAll(" ", "_").substring(0, item.name.length() - 4);
			        		document = openDocument(url);
			        		...
			        		if(item.name.contains("100")) {
				        		...
				        		url = "http://oldschoolrunescape.fandom.com/wiki/" + item.name.replaceAll(" ", "_").substring(0, item.name.length() - 3);
				        		item.forceUntradable = true;
			        		}
			        		document = openDocument(url);
			        	} else if(item.name.contains("- pt")) {
			        		url = "http://oldschoolrunescape.fandom.com/wiki/" + item.name.substring(0, item.name.length() - 6).replaceAll(" ", "_");
			        		document = openDocument(url);
			        	} else if(item.name.contains("(m1)") || item.name.contains("(m2)") || item.name.contains("(m3)") || item.name.contains("(m4)") || item.name.contains("(10)")) {
			        		url = "http://oldschoolrunescape.fandom.com/wiki/" + item.name.substring(0, item.name.length() - 4).trim().replaceAll(" ", "_");
			        		document = openDocument(url);
			        		...
			        		url = "http://oldschoolrunescape.fandom.com/wiki/" + item.name.substring(0, item.name.length() - 3).trim().replaceAll(" ", "_");
			        		try {
						        document = openDocument(url);		
			        		...
			        		url = "http://oldschoolrunescape.fandom.com/wiki/" + item.name.substring(0, item.name.length() - 2).trim().replaceAll(" ", "_");
			        		document = openDocument(url);
			        	} else if(item.name.endsWith("cape(t)")) {
			        		url = "http://oldschoolrunescape.fandom.com/wiki/" + item.name.substring(0, item.name.length() - 3).trim().replaceAll(" ", "_").replaceAll("Woodcut.", "Woodcutting");
			        		document = openDocument(url);
			        	} else if(item.name.contains("hat and")) {
			        		url = "http://oldschoolrunescape.fandom.com/wiki/" + item.name.replaceAll("hat and", "hat %26").replaceAll(" ", "_");
			        		document = openDocument(url);
			        	} else if(item.name.contains(" (uncharged)")) {
			        		url = "http://oldschoolrunescape.fandom.com/wiki/" + item.name.substring(0, item.name.length() - 12).replaceAll(" ", "_");
			        		document = openDocument(url);
			        	} else if(item.name.contains("Dragon javelin(p")) {
			        		url = "http://oldschoolrunescape.fandom.com/wiki/Dragon_javelin";
			        		document = openDocument(url);
			        	}
			    	...
			    	if(document != null) {
			            BufferedWriter htmlWriter = 
			            	     new BufferedWriter(new OutputStreamWriter(
			            	    		 new FileOutputStream("C:/Users/sgsrocks/Desktop/wika/" +
			            	     url.replace("http://oldschoolrunescape.fandom.com/wiki/", "").replace("/", "").replace("???", "") + ".html"), "UTF-8"));
				        ...
				        for (org.jsoup.nodes.Element infoBox : list) {
				        	...
				        	if(examineNext) {
				        		...
				        		if(text.contains("Members only")) {
			        					...
			        					switch(slotElement.last().attr("title").replaceAll(" slot table", "")) {
			        						case "Head":
			        							...
			        							if((item.name.toLowerCase().contains("helm") && item.name.toLowerCase().contains("full"))
			        									|| item.name.toLowerCase().contains("hood")) {
			        								item.helm = true;
				        			...
				        			if (descr != "" && !(descr.toLowerCase().contains("enchant") && (item.slot == 2 || item.slot == 12))) {
...
//				        					System.out.println("----");
			    	...
			    	if(item.id != -1) {
			    		...
			    		json.name("noteable");
			    		json.value(!item.note && item.noteable);
			    		json.name("tradable");
			    		json.value(item.tradable);
			    		json.name("wearable");
			    		json.value(item.equipable);
			    		json.name("showBeard");
			    		json.value(item.helm && !item.name.toLowerCase().contains("med"));
			    		json.name("members");
	...
	private static org.jsoup.nodes.Document openDocument(String url) throws IOException {
		File input = new File("C:/Users/sgsrocks/Desktop/wika/" +
       	     url.replace("http://oldschoolrunescape.fandom.com/wiki/", "").replace("/", "").replace("???", "") + ".html");
		if (input.exists()) {
