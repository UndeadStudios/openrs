...
public class Tracks {
	...
	public static void initialize(Cache cache) {
		// track1s
		try {
			ReferenceTable table = cache.getReferenceTable(CacheIndex.music);
			...
			for (int i = 0; i < table.capacity(); i++) {
				...
				Container container = cache.read(CacheIndex.music, i);
		...
		// tracks2s
		try {
			ReferenceTable table = cache.getReferenceTable(CacheIndex.Jingles);
			...
			for (int i = 0; i < table.capacity(); i++) {
				...
				Container container = cache.read(CacheIndex.Jingles, i);
